...
the member name. */
SELECT DISTINCT  facility,sub.name

FROM (
    SELECT fac.name as facility, CONCAT(mem.firstname,' ', mem.surname) as name
    ...
    WHERE fac.facid = 0 OR fac.facid = 1
    ) sub
...
WHERE starttime LIKE '2012-09-14%' AND	
  CASE WHEN book.memid = 0 THEN 
    fac.guestcost*book.slots >30 
  ELSE
    fac.membercost*book.slots >30
  END
